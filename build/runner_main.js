...
var init_unsafe = __esm({
  async "src/nodes/features/unsafe.ts"() {
    ...
    await init_lipsShim();
    init_all();
    defFeature("unsafe-code", new Feature(async (app2) => {
...
async function loadAllNodes(app2) {
  ...
  for (var node of NODES) {
    app2.defineNode(node);
  }
  for (var [name, fun] of FEATURES) {
    app2.defineFeature(name, fun);
  }
  console.log("Initialized nodes and features");
  console.log(app2.nodeDefs, app2.features);
}
...
var LynxMultiError = class extends LynxError {
  subErrors;
...
function toError(name, err) {
  const f = err instanceof LynxError && err.line !== void 0 && err.col !== void 0 ? `(${err.line}:${err.col}) ` : "";
    ...
    switch (name) {
      case "define":
        if (rest === _nil || rest.length() < 2) {
          allErrors.push(makePosError("Truncated definition", first, 50 /* BAD_SYNTAX */));
          continue;
        }
      ...
      case LINK_COMMAND_NAME:
        const { chains, errors: errors2 } = processSpecialsAndCreation(consToArray(rest));
        chains1.push(...chains);
...
function processSpecialsAndCreation(elements) {
  const chains = [];
  var curZip = [];
  const errors = [];
...
function processNamed(chain, named) {
  ...
  for (var i = 0; i < chain.length; i++) {
    ...
    if (val instanceof LSymbol) {
      ...
      if (reffed === void 0) {
        errors.push(makePosError(
          `Named node ${val} not defined, ignoring this.`,
          val,
          30 /* UNDEFINED_NAME */
        ));
        chain.splice(i, 1);
  ...
  outer: for (var chain of chains) {
    ...
    while (i < chain.length) {
      ...
      connections.push({
        from: prev,
        to: cur,
        portNames: slice.filter((x) => x instanceof PortRef),
        busIndexes: slice.filter((x) => x instanceof LNumber)
      });
      prev = cur;
      i = j;
    }
...
async function autoload(app2, nothingEl) {
  const errors = [];
...
bind("#re-edit", "click", async () => {
  window.location = "../editor" + location.search + location.hash;
});
var app = new LynxFlow(get("#app"), get("#title"), get("#log"), get("#connect-area"));
await loadAllNodes(app);
var noEl = get("#nothingToSeeHere");
noEl.remove();
noEl.style.cssText = "";
var appDef = await autoload(app, noEl);
...
try {
  ...
  if (e75 instanceof LynxMultiError) {
    for (ee of onlyWorstErrors(e75.subErrors)) {
      console.error(ee);
      app.addUI(toError("Init error: ", ee));
    }
  } else {
    console.error(e75);
    app.addUI(toError("Init error: ", e75));
  }
}
var ee;
